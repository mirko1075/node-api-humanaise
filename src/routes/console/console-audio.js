...
router.post('/transcribe', async (req, res) => {
  ...
  try {
    ...
    await knex('files').where({ path: fileUrl }).update({
      ...
      transcription_file_path: whisperTxtFileUrl
    })
    res.json(responseObject)
  } catch () {
    await knex('files').where({ path: fileUrl }).update({
...
router.post('/translate-text', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(txtFilePath)) fs.unlinkSync(txtFilePath) // Remove the temporary .txt file
      if (fs.existsSync(tempFilePath)) fs.unlinkSync(tempFilePath)
      await knex('files').where({ path: fileUrl }).update({
        translation_status: 'failed'
      })
      res.status(500).json({ error: 'Failed to translate the text with Whisper.' })
    }
    ...
    res.status(500).json({ error: 'Failed to translate the text with Google.' })
  } finally {
    // Cleanup
    if (fs.existsSync(txtFilePath)) fs.unlinkSync(txtFilePath) // Remove the temporary .txt file
    if (fs.existsSync(tempFilePath)) fs.unlinkSync(tempFilePath)
